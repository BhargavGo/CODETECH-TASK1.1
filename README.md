...
2. Defining the LED Pin
    ...
    - #define LED_PIN PB5:
      ...
      + Explanation: On an Arduino Uno (which uses an ATmega328P microcontroller), the built-in LED is connected to PB5, which corresponds to digital pin 13. Using a macro
   ...
   _delay_ms(1000);            // Delay for 1000 milliseconds
    ...
    - _delay_ms(1000):
      ...
      + Usage: Keeps the LED in its current state (on or off) for a visible duration.
         ...
         + Note: The _delay_ms() function is a blocking delay, meaning the CPU does nothing else during this time. For simple applications like blinking an LED, this is 
                 acceptable. However, for more complex systems, non-blocking delays or timers are preferred.

8. Turning the LED Off

   ORTB &= ~(1 << LED_PIN);  // Set PB5 low
